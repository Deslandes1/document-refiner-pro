...
def build_html_document(title, body_text, bg, text_col, heading_col, font, for_pdf=False):
    ...
    # Structural Fix: table-layout: fixed & explicit widths ensure the table behaves inside bounded spaces
    header_html = f"""
...
<body><div class="document-content">{header_html}<div style="font-size: 11pt; line-height: 1.5; width: 100%;">{escaped_body}</div></div></body>
</html>"""
    else:
        return f"""<!DOCTYPE html>
<html>
...
st.markdown("---")
st.markdown(f"<div style='color: {text_color}; font-family: {font_family}; white-space: pre-wrap; word-wrap: break-word;'>{active_payload}</div>", unsafe_allow_html=True)
